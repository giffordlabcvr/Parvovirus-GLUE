...
process_alignment_tree("AL_MASTER_Dependoparvovirus");
...
function process_feature(featureSummary, refseqID, featureID) {
	...
	if (featureID == "orf" || featureID == "NS5" ) {
...
function create_feature_fasta(refseqID, featureID, featureCodons, featureSummary) {
	...
	if (featureID == "NS") {
	  orf_fasta_aa.push(fasta_aa); 
	  orf_fasta_nt.push(fasta_codons);
...
// write feature fasta 
function write_feature_fasta(orf_fasta_aa) {

	
	var orf_fasta_aa_str = orf_fasta_aa.join("\n");
