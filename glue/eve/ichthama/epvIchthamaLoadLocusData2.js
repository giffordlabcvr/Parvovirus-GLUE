var refconDataPath = "tabular/eve/hamaparvovirinae/epv-ichthama-refseqs-side-data.tsv";
...
glue.inMode("module/tabularUtility", function() {
	loadResult = glue.tableToObjects(glue.command(["load-tabular", "tabular/eve/hamaparvovirinae/epv-ichthama-side-data.tsv"]));
...
_.each(loadResult, function(eveObj) {
	...
	if (eveObj.locus_name != 'NK') { // Skip elements that haven't been assigned to a locus
		...
		glue.inMode("sequence/"+source_name+"/"+eveObj.sequenceID, function() {
			...
			glue.command(["set", "field", "genus", "Ichthamaparvovirus"]);
			glue.command(["set", "field", "subfamily", "Hamaparvovirinae"]);
