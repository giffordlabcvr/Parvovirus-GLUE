var refconDataPath = "tabular/eve/parvovirinae/epv-unc-parvo-refseqs-side-data.tsv";
var source_name = "fasta-digs-unc-parvo-epv";
...
glue.inMode("module/tabularUtility", function() {
	loadResult = glue.tableToObjects(glue.command(["load-tabular", "tabular/eve/parvovirinae/epv-unc-parvo-side-data.tsv"]));
...
_.each(loadResult, function(eveObj) {
	...
	if (eveObj.locus_name != 'NK') { // Skip elements that haven't been assigned to a locus
		...
		glue.inMode("sequence/"+source_name+"/"+eveObj.sequenceID, function() {
			...
			glue.command(["set", "field", "genus", "Unclassified"]);
